...
public class CompoundInterest {
    ...
    static double futureValue(double presentValue, double rate,
                              int targetYear) {
        double use_rate = (rate + 100)/100;
        return presentValue * Math.pow(use_rate,numYears(targetYear) ) ;
    }
    ...
    static double futureValueReal(double presentValue, double rate,
        ...
        double use_inflation = (100 - inflationRate)/100;
        return CompoundInterest.futureValue(presentValue, rate, targetYear) * Math.pow(use_inflation,numYears(targetYear) );
    }
    ...
    static double totalSavings(double perYear, int targetYear, double rate) {
        double total = 0;
        double years_to_loop = numYears(targetYear);
        while (years_to_loop >= 0) {
            total = total + perYear* Math.pow(1 + rate/100, years_to_loop );
            years_to_loop = years_to_loop - 1;
        }
    ...
    static double totalSavingsReal(double perYear, int targetYear, double rate,
        ...
        double inflation_use = 1 - inflationRate/100;
        return totalSavings(perYear, targetYear, rate) * Math.pow(inflation_use, numYears(targetYear));
